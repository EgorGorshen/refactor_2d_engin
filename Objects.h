...
class Charge : public Object
{
  ...
  public:
    void init(Mesh m, int fm, int x, int y, int z, float rot, float sc, std::string tgs, std::string arg,
              std::string tp) override
    {
        this->X = x;
        this->Y = y;
        this->layer = z;
    ...
    std::string update(double delta, std::string task = "", std::string arguments = "") override
    {
        ...
        else
        {
            for (auto i : ActiveWorld.objects)
            {
                ...
                {
                    double pq = std::stod(i.get()->update(0, "getq", ""));
                    Vector r = Vector(X, Y, i.get()->GetLocation().X, i.get()->GetLocation().Y).normalise();
                    double rl = Vector(X, Y, i.get()->GetLocation().X, i.get()->GetLocation().Y).length();
                    Vector a;
                    if (rl != 0)
                    {
                        a = Vector((pq * q * 9 * pow(10, 9) / rl * rl * r.X) / m,
                                   (pq * q * 9 * pow(10, 9) / rl * rl * r.Y) / m);
                    }
            ...
            if (c > 0)
            {
                meshes[0].change_color(0, 0, 255, 255);
            }
            else if (c == 0)
            {
                meshes[0].change_color(255, 255, 255, 255);
            }
            else
            {
                meshes[0].change_color(255, 0, 0, 255);
            }
            return "";
        }
    }
