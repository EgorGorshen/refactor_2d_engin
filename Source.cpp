...
int main()
{
    ...
    /*
    ObjectPointer a = ActiveWorld.spawn<Character>(CreateRect(-20, -20, 20, 20, 0,
    255, 0, 255), 250, 539, 0, 1, "phys", "chr");
    ...
    ActiveWorld.spawn<Object>(CreateRect(-400, -20, 400, 20, 255, 0, 0, 255), 400,
    580, 0, 1, "phys", "mpl"); for (int i = 0; i < 6; i++) {
        ActiveWorld.spawn<Object>(CreateRect(-60, -20, 60, 20, 255, 0, 0, 255), 70
    + rand() % 530, 60 + 85 * i, 0, 1, "phys", "pl");
    }
    */
    ...
    for (int i = 0; i < r; i++) {
        ...
        getline(str, n);
        double q = stod(n);
        ActiveWorld.spawn<Charge>(CreateNangle(8, 10, 0, 0, 255, 255), x, y,
    0, 1.4, "phys", std::to_string(i)).get()->update(0, "setq",
    std::to_string(q));
    }
    */

    // ActiveWorld.spawn<Object>(CreateRect(-20, -20, 20, 20, 255, 0, 0, 255), 40,
    // 40, 0, 1, "phys", "lox1"); ActiveWorld.spawn<Object>(CreateRect(-20, -20,
    // 20, 20, 0, 0, 255, 255), 90, 90, 0, 1, "phys", "lox2");

    /*
    for (int i = 0; i < 80; i++) {
        ActiveWorld.spawn<Object>(CreateNangle(rand() % 9 + 1, 40, rand() % 255,
    rand() % 255, rand() % 255, 255), rand() % 1024, rand() % 1024, rand() % 360,
    1, "phys", "lox2");
    }
    */
    ...
    while (window.isOpen() and not(Animate))
    {
        ...
        {
            if (i < ActiveWorld.objects.size())
                ActiveWorld.objects[i].get()->update(cl.getElapsedTime().asSeconds());
        }
        if (!once)
        {
        ...
        while (window.pollEvent(event))
        {
            ...
            if (event.type == sf::Event::KeyPressed)
            {
                ...
                if (event.key.code == sf::Keyboard::R) {
                    ...
                    for (int i = 0; i < 6; i++) {
                        ActiveWorld.spawn<Object>(CreateRect(-60, -20, 60, 20, 255, 0,
                0, 255), 70 + rand() % 530, 60 + 90 * i, 0, 1, "phys", "pl");
                    }
                }
                */
            }
        ...
        */
        ...
        sf::Keyboard::isKeyPressed(sf::Keyboard::Space)) { ya -= 2;
        }
        ActiveWorld.FindByName("chr").get()->update(0, "control", std::to_string(xa)
        + " " + std::to_string(ya));
        */
        // ActiveWorld.FindByName("lox2").get()->ChangeLocation(sf::Mouse::getPosition().x
        // - window.getPosition().x, sf::Mouse::getPosition().y -
        // window.getPosition().y);
        // SimpleBoxCollision(ActiveWorld.objects[0].get()->GetPoints(0),
        // ActiveWorld.objects[1].get()->GetPoints(0));
        /*
        if (ActiveWorld.FindByName("chr").get()->GetLocation().Y < -10) {
            ...
            for (int i = 0; i < 6; i++) {
                ActiveWorld.spawn<Object>(CreateRect(-60, -20, 60, 20, 255, 0, 0,
        255), 70 + rand() % 530, 60 + 90 * i, 0, 1, "phys", "pl");
            }
        ...
        */
        ...
        for (ObjectPointer o : ActiveWorld.objects)
        {
            for (int b = 0; b < o.get()->GetMeshesCount(); b++)
            {
                ...
                sh.setFillColor(sf::Color(o.get()->GetMesh(b)->color[0], o.get()->GetMesh(b)->color[1],
                                          o.get()->GetMesh()->color[2], o.get()->GetMesh()->color[3]));
